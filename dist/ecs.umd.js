/**
 * -----------------------------------------------------------
 * Entity Component System Framework v1.0.0
 * 
 * Copyright(C) didix16
 * https://github.com/didix16
 * -----------------------------------------------------------
 */
...
(function (global, factory) {
    ...
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.ecs = {}, global.uuid));
    ...
    ***************************************************************************** */
    ...
    var ComponentManager = /** @class */ (function (_super) {
        ...
        ComponentManager.prototype.find = function (name) {
            return this.components[name] ? lodash.clone(this.components[name]) : null;
        };
        /**
        ...
        ComponentManager.prototype.build = function (name, data) {
            ...
            if (lodash.isUndefined(data) || lodash.isEmpty(data))
                return comp;
            return lodash.assign(comp, lodash.pick(data, lodash.keys(comp)));
        };
        /**
        ...
        ComponentManager.prototype.component = function (name, data) {
            ...
            return lodash.clone(this.components[name]);
        };
        /**
    ...
    var EntityManager = /** @class */ (function (_super) {
        ...
        EntityManager.prototype.findByComponents = function (components) {
            return lodash.filter(this.all(), function (entity) {
                return lodash.size(lodash.pick(entity.components, components)) === components.length;
            });
        };
        /**
        ...
        EntityManager.prototype.addComponent = function (entity, component, data) {
            ...
            if (!this.hasComponent(entity, component)) {
                this.entities[entity].components[component] = build(component, data);
    ...
    var System = /** @class */ (function () {
        ...
        System.prototype.update = function () {
            var _this = this;
            lodash.each(this.entities, function (entity) {
                _this.handle(entity.id, lodash.pick(entity.components, _this.requiredComponents));
            });
        };
        return System;
    }());
    ...
    var SystemManager = /** @class */ (function (_super) {
        ...
        SystemManager.prototype.findByComponents = function (components) {
            return lodash.filter(this.all(), function (system) {
                return lodash.intersection(system.requiredComponents, components).length === components.length;
            });
        };
        /**
        ...
        SystemManager.prototype.validEntity = function (system, entity) {
            ...
            return lodash.size(lodash.pick(entity.components, requiredComps)) === reqCompsLen;
        };
        /**
    ...
    var ECS = /** @class */ (function (_super) {
        ...
        function ECS() {
            ...
            _this.em = EntityManager.getInstance();
            _this.cm = ComponentManager.getInstance();
            _this.sm = SystemManager.getInstance();
        ...
        ECS.prototype.registerEvents = function () {
            var _this = this;
            this.em.on(EntityManager.EV_ENTITY_CREATED, function (entityId) {
                _this.debug("entity created: " + entityId);
            });
            this.em.on(EntityManager.EV_ENTITY_DELETED, function (entity) {
                // remove entity from all posible systems
                _this.sm.all().forEach(function (system) {
                    _this.sm.dettachEntity(system.name, entity);
                });
            });
            this.em.on(EntityManager.EV_COMPONENT_ADDED, function (entityId, componentName) {
            ...
            this.em.on(EntityManager.EV_COMPONENT_REMOVED, function (entityId, componentName) {
                // remove the entity from all systems that use that component
                _this.sm.findByComponents([componentName]).forEach(function (system) {
                    _this.sm.dettachEntity(system.name, _this.em.find(entityId));
                });
            });
            this.cm.on(ComponentManager.EV_COMPONENT_CREATED, function (componentName) {
                _this.debug("component created: " + componentName);
            });
            this.cm.on(ComponentManager.EV_COMPONENT_DELETED, function (componentName) {
            ...
            this.sm.on(SystemManager.EV_SYSTEM_CREATED, function (systemName) {
                _this.debug("system created: " + systemName);
                // attach those entitites that are compatible with the new system
                _this.em.findByComponents(system(systemName).requiredComponents).forEach(function (entity) {
                    _this.sm.attachEntity(systemName, entity);
                });
            });
            this.sm.on(SystemManager.EV_SYSTEM_DELETED, function (systemName) {
                _this.debug("system deleted: " + systemName);
            });
            this.sm.on(SystemManager.EV_ENTITY_ATTACHED, function (entityId, systemName) {
                _this.debug("entity " + entityId + " attached to " + systemName + " system");
            });
            this.sm.on(SystemManager.EV_ENTITY_DETTACHED, function (entityId, systemName) {
         ...
         */
        ECS.prototype.attachEntity = function (system$1, entity$1) {
            if (!this.sm.exists(system$1)) {
                throw new Error("System " + system$1 + " does not exists!");
            }
            var e = entity(entity$1);
            if (!e)
                throw new Error("Entity " + entity$1 + " does not exists");
            var result = this.sm.attachEntity(system$1, e);
            if (!result) {
                var components = system(system$1).requiredComponents.join(",");
        ...
        ECS.prototype.dettachEntity = function (system, entity$1) {
...
//# sourceMappingURL=ecs.umd.js.map
